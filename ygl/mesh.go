...
func (m *Mesh) Setup() {
	if m.Positions.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[POS_M_VB], m.Positions.Len(), gl.Ptr(&pos[0]), 0)
	...
	if m.Normals.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[NORMAL_M_VB], m.Normals.Len(), gl.Ptr(&normals[0]), 0)
	...
	if m.TexCoords.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[TEXCOORDS_M_VB], m.TexCoords.Len(), gl.Ptr(&texs[0]), 0)
	...
	if m.TexCoords2.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[TEXCOORDS2_M_VB], m.TexCoords2.Len(), gl.Ptr(&texs[0]), 0)
	...
	if m.JointIndices.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[JOINTS_M_VB], m.JointIndices.Len(), gl.Ptr(&joints[0]), 0)
	...
	if m.WeightValues.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[WEIGHTS_M_VB], m.WeightValues.Len(), gl.Ptr(&weights[0]), 0)
	...
	if m.Indices.Len() != 0 {
		...
		gl.NamedBufferStorage(m.Buffers[INDEX_M_VB], m.Indices.Len(), gl.Ptr(&indices[0]), 0)
